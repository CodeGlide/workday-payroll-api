...
func Get_paygroupsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/payGroups%s", cfg.BaseURL, queryString)
...
func CreateGet_paygroupsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_payGroups",
		mcp.WithDescription("Retrieves a collection of pay groups."),
		mcp.WithString("country", mcp.Description("The Workday ID of the country or territory of the pay group. You can use a returned country id from any of the payGroups endpoints in this Payroll API.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_paygroupsHandler(cfg),
	}
}
